          ...
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-2">
              {TRAITS.map((item) => (
                <span
                  key={item}
                  className="inline-flex items-center gap-1.5 rounded-full border border-zinc-200 bg-white px-3 py-1.5 text-[11px] font-semibold text-zinc-800 shadow-card"
                >
            ...
            {PRIMITIVES.map((primitive) => (
              <Panel key={primitive.label} hover className="overflow-hidden">
                ...
                <div className="p-6">
                  ...
                  <p className="mt-4 text-sm leading-6 text-zinc-700">{primitive.body}</p>
                  <ul className="mt-5 space-y-2 rounded-xl bg-paper-200/70 p-3.5">
                    {primitive.lines.map((line) => (
                      <li
                        key={line}
                        className="flex items-center gap-2.5 font-mono text-[11px] text-zinc-700"
                      >
