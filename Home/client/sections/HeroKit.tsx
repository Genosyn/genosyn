...
export function HeroBadgeDot() {
  return <span aria-hidden className="h-1 w-1 shrink-0 rounded-full bg-ink-400" />;
}
...
export function HeroTagline({ children }: { children: ReactNode }) {
  return (
    <p className="mt-5 max-w-xl text-balance text-lg font-semibold leading-8 tracking-[-0.015em] text-ink-600 sm:text-xl">
      {children}
    </p>
  );
}
    ...
    <ul className="mt-10 grid max-w-lg gap-x-6 gap-y-3 text-xs font-medium text-stone-600 sm:grid-cols-2">
      {items.map((item) => (
        <li key={item} className="flex items-center gap-2.5">
          <span
            aria-hidden
            className="flex h-5 w-5 shrink-0 items-center justify-center rounded-md bg-ink-100 text-ink-600"
          >
...
export function HeroPanel({
  ...
  return (
    <div className="relative">
      <div
        aria-hidden
        className="pointer-events-none absolute -inset-10 -z-10 rounded-[3rem] bg-[radial-gradient(60%_55%_at_50%_40%,rgba(255,178,122,0.16),transparent_72%)] blur-2xl"
      />
