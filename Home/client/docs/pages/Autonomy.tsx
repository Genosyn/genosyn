...
export function Autonomy() {
  return (
    <>
      ...
      <P>
        Once an hour, a sweep reads each employee&apos;s trailing <Strong>30-day</Strong> record.
        To be eligible for a waiver, the record must show:
      </P>
      <UL>
        <LI>
          At least <Strong>10</Strong> terminal Runs, with <Strong>zero</Strong> failures or
          timeouts, nothing graded{" "}
          <DocLink to="/docs/routines#outcome-check">off goal</DocLink>, and no required{" "}
          <DocLink to="/docs/verification">Check</DocLink> failed.
        </LI>
        <LI>
          ...
          as &quot;fine&quot;, which meant an outage in the checker could earn an employee the
          right to work unattended.
        </LI>
        <LI>
      ...
      <P>
        ...
        evidence — they are a hundred loops that returned. Give the Routine a bar first, then let
        it clear it.
      </P>
      <P>
        An eligible employee gets no silent upgrade. The sweep drafts an{" "}
        <Strong>Approval</Strong> of kind <Code>autonomy_promotion</Code>, with the evidence — the
        run counts, the approval tallies, the window — written into its summary, and it lands in
        the same Approvals inbox as everything else. An admin approving it is what applies the gate
        change and records the waiver; rejecting it means the same promotion is not proposed again
        for <Strong>30 days</Strong>.
      </P>

      <H2 id="demotion">Demotion is automatic</H2>
      <P>
        The reverse direction has no inbox and no waiting. Any Run that ends{" "}
        <Code>failed</Code> or <Code>timeout</Code>, or completes but is graded{" "}
        <Code>off goal</Code>, or completes with a required{" "}
        <DocLink to="/docs/verification">Check</DocLink> failed, revokes <Strong>every</Strong>{" "}
        active waiver the employee holds and re-arms the gates on the spot. A failed Check counts
        here exactly like an off-goal grade — it is the stronger evidence of the two, since no model
        had a say in it. The employee&apos;s journal records what happened and why,
        and owners, admins, and the employee&apos;s manager are paged. Demotion only ever tightens
        — a bad Run can take autonomy away, never hand more out.
      </P>
      ...
      <P>
        ...
        happening at all, the instrument is a <DocLink to="/docs/standdowns">Standdown</DocLink> —
        a waiver&apos;s exact inverse, imposed rather than earned, and broad rather than narrow.
      </P>
