  ...
  return (
    <>
      <PageHeader
        ...
        lead={
          <>
            An AI Employee can read a <Code>.docx</Code>, answer it, and hand back the same document
            — the customer&apos;s own formatting intact, with answers in it. It can write a new one
            from scratch when the deliverable is a document rather than a message, and convert one
            to PDF when the next step needs a PDF.
          </>
        }
      />
      ...
      <P>
        It returns every paragraph and table cell with an id, alongside any form fields the document
        declares. Paragraph ids run <Code>p1</Code>, <Code>p2</Code>, … in document order; table
        cells are <Code>t1r2c3</Code>; anything outside the main body carries its part as a prefix,
        like <Code>header1:p2</Code>. Headers, footers, footnotes and comments are read by default,
        because a questionnaire&apos;s answer boxes are often in a header and a reader that skipped
      ...
      <UL>
        <LI>
          <Code>set_paragraph</Code> replaces a paragraph&apos;s text, keeping the run formatting it
          already had. This is how an answer goes onto a blank line.
        </LI>
        <LI>
          <Code>insert_paragraph</Code> adds paragraphs beside an existing one and inherits its
          formatting, so a bullet added under an answer is a bullet. Pass an array to add several at
          once.
        </LI>
        <LI>
...
// → { attachment: { id, filename: "MSS_blank-edited.docx" }, applied: [...] }`}</Pre>
      ...
      <Callout kind="warn" title="One bad id refuses the whole batch">
        ...
        nothing changes and all the problems come back together. A run of eight answers that quietly
        skipped the two with wrong ids would hand a human a questionnaire that looks finished and is
        not — which costs far more to recover from than a refusal.
      </Callout>

      <H2 id="creating">Writing a new one</H2>
      <P>
        <Code>create_docx</Code> builds a document from Markdown. Headings, bullet and numbered
        lists, tables, quotes, code blocks, bold, italic and links all become real Word constructs —
        a heading is <Code>Heading1</Code>, a list carries a numbering definition, a table is a
        table — so the recipient gets something they can restyle and keep working in rather than a
        text file with hashes in it.
      </P>
      <Pre>{`create_docx({
...
// → { attachment: { id, filename: "q3-security-review.docx" } }`}</Pre>
      ...
      <P>
        ...
        edit. <Code>convert_to_pdf</Code> takes an <Code>attachmentId</Code> and returns a PDF
        attachment.
      </P>
