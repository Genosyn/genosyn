    ...
    <>
      ...
      <UL>
        ...
        <LI>
          ...
          members and AI Employees, under each company&apos;s{" "}
      ...
      <P>
        ...
        recorded in the background so nothing is missing when you upgrade. On Genosyn Cloud the
        same two features come with the Scale plan instead — see{" "}
        <DocLink to="/docs/plans-billing" />.
      </P>

      <H2 id="getting-a-license">Getting a license</H2>
      <P>
        Write to{" "}
        <ExtLink href="mailto:enterprise@genosyn.com">enterprise@genosyn.com</ExtLink> or start
        from <ExtLink href="https://genosyn.com/pricing">genosyn.com/pricing</ExtLink>. You receive
        a single license key — a long string starting with <Code>genlic1.</Code> — sized to your
        company, with an expiry date and optionally a seat count.
      </P>
      ...
      <KeyList
        rows={[
          ...
          },
          {
            ...
            def: (
              <>
                How many AI Employees the license covers, next to how many are in use — for
                example &ldquo;Licensed for 25 AI Employees · 12 in use&rdquo;. This is
                informational: the software never blocks a hire over it. A license without a seat
                count shows &ldquo;Unlimited AI Employees&rdquo;.
              </>
            ),
          },
          {
      ...
      />
      <P>
        <Strong>Remove license</Strong> returns the install to Community edition: SSO and the
        Audit log turn off until a license is activated again, and nothing is deleted.
      </P>
      <P>
      ...
      <Callout kind="tip" title="Offline by design — air-gapped installs welcome.">
        ...
        the software. There is no phone-home, no activation server, and no network requirement —
        a key pasted into an air-gapped install validates exactly like one on the open internet.
      </Callout>
      ...
      <P>
        ...
        holds the Ed25519 signing private key — in practice, genosyn.com&apos;s own cloud. A
        keypair is generated with <Code>npm run license:keygen</Code>; the private key is stored
        encrypted and never shown again. Issuing a license returns the full signed key exactly
        once — only a masked preview is kept in the issued-licenses table — so it is copied and
        delivered to the customer on the spot.
      </P>
