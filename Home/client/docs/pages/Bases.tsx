  ...
  return (
    <>
      <PageHeader
        ...
        lead={
          <>
            Airtable-style structured data built in: a <Strong>Base</Strong> holds tables of records
            with typed columns, saved views, comments, and file attachments — editable by your team
            and by granted AI employees. Find it under <Strong>Bases</Strong> in the section menu.
          </>
        }
      />
        ...
        rows={[
          {
            ...
            def: (
              <>
                Finance customers — see <DocLink to="/docs/customers" />. A deals table with a
                Customer column keeps CRM rows attached to the account you invoice.
              </>
            ),
          },
          {
            ...
            def: (
              <>
                Invoices from <DocLink to="/docs/finance" /> — drafts show as <Code>Draft …</Code>,
                issued ones by their number.
              </>
            ),
          },
          {
            ...
            def: (
              <>
                Projects from <DocLink to="/docs/tasks" />. Restricted projects only appear to
                people who can already open them.
              </>
            ),
          },
          {
      ...
      />
      <P>
        Cells can hold one or many linked records. Each shows as a chip; clicking a chip jumps
        straight to that customer, invoice, project, note, or pipeline. Archived customers and notes
        stay resolvable in existing cells but are hidden from the picker, so new links always point
        at live records.
      </P>
      <Callout kind="tip" title="AI Employees see the same links">
        ...
        — so a routine can, say, file every new deal against the right finance customer without you
        mapping ids by hand.
      </Callout>
      ...
      <UL>
        <LI>
          <Strong>Fields</Strong> — click any value to edit in place; the same editors as the grid,
          including the record-link pickers.
        </LI>
        <LI>
          <Strong>Files</Strong> — upload up to 25&nbsp;MB per file; AI employees can attach
          evidence via <Code>attach_file_to_record</Code>.
        </LI>
        <LI>
          <Strong>Comments</Strong> — humans and AI Employees share one thread, so a routine&apos;s
          findings land next to your notes.
        </LI>
      </UL>

      <H2 id="views">Views, filters, and sorts</H2>
      <P>
        Each table has saved <Strong>views</Strong> — tabs above the grid that remember filters,
        sorts, and hidden fields. Filter operators adapt to the column type; record-link columns
        filter with <Code>has any of</Code> / <Code>has none of</Code> against the linked records.
      ...
      <P>
        ...
        files — for active tables only. Archived tables are hidden and inaccessible until a Member
      ...
      <Callout kind="info" title="Templates">
        ...
        tracker — with tables, fields, and example rows pre-seeded.
      </Callout>
    </>
  );
}
