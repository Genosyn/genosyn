  ...
  return (
    <>
      ...
      <P>
        ...
        <DocLink to="/docs/goals">Goal</DocLink> updates — the Routine fires without waiting for
        its next cron slot. Triggers are managed by admins on the routine&apos;s{" "}
        <Strong>Settings → Triggers</Strong> card, and the list of subscribable kinds is served
        from the same registry the app&apos;s own live updates run on — anything that refreshes on
        your screen can fire a Routine.
      </P>
      <Callout kind="info" title="An event routes work. It never carries content.">
        Event frames are coarse and <Strong>id-only</Strong>: a fire tells the Routine only that
        its subscribed family changed. The employee then reads the actual state through its own
        grant-gated tools, the same way it would on a cron tick — so a Trigger decides{" "}
        <em>when</em> work happens, and never smuggles data past a{" "}
      ...
      <UL>
        <LI>
          ...
          <DocLink to="/docs/routines#approvals">Approval</DocLink> a cron tick would — the
          webhook precedent, verbatim. An event is never a bypass.
        </LI>
        <LI>
          <Strong>A minimum interval bounds every Trigger</Strong> — 15 minutes by default, floor
          of 1 minute. However many changes land inside the window, the Routine fires at most
          once, so a routine that writes the very family it subscribes to converges to one fire
      ...
      <Callout kind="warn" title="A Trigger is not a Signal.">
        ...
        your own database. A Trigger is a change subscription on Genosyn&apos;s own resources.
        Same reflex, different words — deliberately.
      </Callout>
      ...
      <P>
        ...
        &quot;check back on the invoice in two days&quot; — using the{" "}
        <Code>schedule_wakeup</Code> tool, with a note its future self will read (and{" "}
        <Code>cancel_wakeup</Code> when the follow-up becomes moot). At the time named, a fresh
        session starts under the employee&apos;s own authority, briefed with that note. The
        session&apos;s report lands on the wakeup and in the employee&apos;s journal, so a timer
        never fires into silence — and if no AI Model is connected when it comes due, the note
        itself is delivered to the journal instead of being lost. Pending wakeups show in a card
      ...
      <P>
        A <Strong>Workstream</Strong> is a persistent state document for work that spans many
        Runs — a migration, a long negotiation, a multi-week cleanup. The employee maintains it
        with <Code>create_workstream</Code>, <Code>update_workstream</Code>, and{" "}
      ...
      <Callout kind="info" title="A Workstream is not a Project.">
        ...
        assigned, tracked. A Workstream is one employee&apos;s own working state, written by it
        and read back to it. Humans can read a workstream; they don&apos;t work out of it.
      </Callout>
      ...
      <KeyList
        rows={[
          { term: "title", def: "What the initiative is called. Duplicate pending titles are refused." },
          {
            term: "routine",
            def: (
              <>
      ...
      <P>
        ...
        section under the AI nav. <Strong>Accepting</Strong> creates precisely the Routine
        proposed — owned by the proposing employee, scheduled immediately.{" "}
        <Strong>Declining</Strong> journals the reason back to the employee, so the next proposal
        is better aimed. At most <Strong>5</Strong> initiatives can be pending per employee.
      </P>
      <Callout kind="info" title="Nothing exists until a human accepts.">
        ...
        schedule its own idea into existence — the accept click is what creates the Routine, and
        it creates exactly what was proposed, nothing more.
      </Callout>
    </>
  );
}
