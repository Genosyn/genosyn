...
export function DocsShell({ pathname, children }: { pathname: string; children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-paper-50 text-stone-800">
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="flex flex-col gap-10 lg:flex-row lg:gap-12">
          <aside
            ...
            className={`fixed inset-y-0 left-0 z-40 w-72 transform overflow-y-auto border-r border-stone-900/[0.08] bg-paper-50 px-6 pb-10 pt-20 transition-transform lg:sticky lg:top-16 lg:z-auto lg:h-[calc(100vh-4rem)] lg:w-60 lg:flex-shrink-0 lg:translate-x-0 lg:border-r-0 lg:px-0 lg:pb-12 lg:pt-10 ${
    ...
    <nav className="space-y-7 text-sm">
      {DOCS_NAV.map((section) => (
        <div key={section.label}>
          <div className="px-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-stone-500">
            {section.label}
          </div>
                ...
                <li key={page.path}>
                  <Link
                    ...
                    className={`block rounded-md px-2 py-1.5 text-[14px] leading-snug transition ${
                      active
                        ? "bg-paper-200 font-medium text-stone-800"
...
function PrevNext({ prev, next }: { prev: DocsPageMeta | null; next: DocsPageMeta | null }) {
  ...
  return (
    <div className="grid grid-cols-1 gap-3 border-t border-stone-900/[0.06] pt-8 sm:grid-cols-2">
      {prev ? (
        <Link
          href={prev.path}
          className="group flex flex-col rounded-xl border border-stone-900/[0.08] bg-white px-5 py-4 text-left shadow-card transition hover:border-stone-900/20"
        >
          <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-[0.18em] text-stone-500">
            <ArrowLeft className="h-3 w-3" />
            Previous
          </span>
          <span className="mt-2 text-[15px] font-semibold text-stone-900">{prev.title}</span>
        </Link>
      ) : (
        <span />
      )}
      {next ? (
        <Link
          href={next.path}
          className="group flex flex-col items-end rounded-xl border border-stone-900/[0.08] bg-white px-5 py-4 text-right shadow-card transition hover:border-stone-900/20"
        >
          <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-[0.18em] text-stone-500">
            Next
            <ArrowRight className="h-3 w-3" />
          </span>
          <span className="mt-2 text-[15px] font-semibold text-stone-900">{next.title}</span>
        </Link>
      ) : (
        <span />
      )}
...
function DocsFooter() {
  return (
    <footer className="border-t border-stone-900/[0.06] bg-white">
      <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-3 px-6 py-8 text-xs text-stone-500 sm:flex-row lg:px-8">
        ...
        <nav className="flex flex-wrap items-center justify-center gap-x-5 gap-y-2">
          ...
          <a href="/install.sh" className="hover:text-stone-900">
            install.sh
          </a>
          <span className="font-mono text-stone-400">v{__APP_VERSION__}</span>
        </nav>
      </div>
    </footer>
  );
