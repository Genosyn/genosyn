            ...
            {stories.map((candidate, index) => (
              ...
              >
                ...
                {index === storyIndex && motionEnabled && (
                  <span className="prototype-progress absolute inset-x-0 bottom-0 h-0.5 origin-left bg-indigo-500" />
                )}
              </div>
            ))}
          </div>
...
function PrototypeSidebar({
  ...
  return (
    <aside className="hidden min-w-0 border-r border-slate-200 bg-slate-50 p-3 lg:flex lg:flex-col">
      ...
      <div className="mt-auto rounded-xl border border-indigo-100 bg-indigo-50/70 p-2.5">
        <div className="flex items-center gap-2 text-[9px] font-semibold text-indigo-700">
