...
import { api, CustomerCredit, formatMoney } from "../lib/api";
...
/**
 * Credit notes list. Phase H of the Finance milestone (M19). Credit memos are
 * raised from an invoice's detail page; this is the register of them all.
 */
export default function FinanceCreditNotes() {
  const { company } = useOutletContext<FinanceOutletCtx>();
  const [credits, setCredits] = React.useState<CustomerCredit[] | null>(null);

  const reload = React.useCallback(async () => {
    setCredits(await api.get<CustomerCredit[]>(`/api/companies/${company.id}/credit-notes`));
  }, [company.id]);
    ...
    <div className="mx-auto max-w-6xl p-8">
      ...
      {credits === null ? (
        ...
        <div className="rounded-xl border border-dashed border-slate-300 p-10 text-center text-sm text-slate-500 dark:border-slate-700 dark:text-slate-400">
          No credit notes yet. Raise one from an invoice&apos;s detail page with the{" "}
          <span className="font-medium">Credit note</span> action.
        </div>
      ) : (
              ...
              {credits.map((c) => (
                <tr key={c.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                  <td className="px-4 py-2 font-medium">
                    <Link
                      to={`/c/${company.slug}/finance/credit-notes/${c.slug}`}
