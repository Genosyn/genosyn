...
/**
 * One line on an `Estimate`. Mirrors `InvoiceLineItem` so a single
 * `replaceLines` helper can be shared between estimates and invoices —
 * the only difference is the foreign-key column name.
 *
 * Snapshot semantics: `description`, `unitPriceCents`, `taxName`,
 * `taxPercent`, and `taxInclusive` are all snapshotted from the source
 * `Product` / `TaxRate` at line-create time. Editing the product or tax
 * rate later doesn't change this row — once an estimate has been sent
 * to a customer the line shape is fixed history.
 *
...
export class EstimateLineItem {
  ...
  /** Source product, kept as a reference for "rebill same item" UX.
   *  Lines stay valid if the product is later archived or deleted. */
