...
export const PROACTIVE_RECIPES: ProactiveRecipe[] = [
  ...
  },
  {
    ...
    brief:
      "Determine whether the report concerns code in a granted Repository. If it is a non-code support request, solve it from company knowledge or put an exact targeted clarification in a Decision-stack mail review. For code, inspect existing Workstreams and sessions to avoid duplicate fixes, select the correct granted Repository, and call start_repository_work_session with the observed behavior, expected behavior, relevant non-secret evidence, and a request for a narrow fix with regression tests. Never ask the session to access unrelated systems or reproduce offensive exploits. The session runs asynchronously: save its sessionId and the source thread in a Workstream. Check get_repository_work_session before claiming a fix is ready. In an authorized follow-up, use open_repository_work_session_pull_request only when the session is ready, its evidence is reviewed, the Soul permits publication, and your Repository write Grant remains active. A stored HTTPS personal access token can push and open the PR without a separate Connection Grant; SSH and Connection-backed repositories require an explicit Grant to the Repository's pinned forge Connection. Otherwise leave the ready branch for a Member. Never merge or push the default branch. Put an honest customer update with the verified PR link in the Decision stack when available. A queued work session is not a completed fix.",
    acceptanceCriteria: criteria,
  },
  {
    ...
    brief:
      "Match the sender to an existing Contact by exact email. Create or enrich only facts supported by the message. Find an existing open Deal for this enquiry before creating another; record the source thread as an Activity. Put the exact relevant answer and a short qualification question in a Decision-stack mail review only when needed. Do not enroll the Contact into outbound Sequences or assert marketing consent merely because they emailed. Record the next follow-up in a Workstream.",
    acceptanceCriteria: criteria,
  },
  {
    id: "spam-cleanup",
  ...
  },
  {
    ...
    triggerKind: "invoice",
    description:
      "Check unpaid invoices against recent payments and prepare timely, accurate reminders for Decision-stack review.",
    requirements: ["mail", "financeRead"],
    brief:
      "Review overdue issued invoices, current balances, disputes, and recent customer replies. Never chase a paid, void, disputed, or recently reminded invoice. Put the exact concise reminder, actual invoice PDF, and approved payment instructions in a Decision-stack mail review. Do not send automatically from this starter, change amounts, record imaginary payments, or threaten consequences. Use the Workstream to track last reminder and next due date; default to at least seven days between reminders unless company policy says otherwise.",
    acceptanceCriteria: criteria,
  },
  {
    id: "stalled-deals",
    ...
    requirements: ["mail", "revenueWrite"],
    brief:
      "Review open Deals with no next action or a follow-up due. Read recent Activities and email before proposing contact. Update factual Deal details, create a next step, and put an exact personalized follow-up in a Decision-stack mail review only where there is an existing relevant relationship. Respect Suppressions and do not send or enroll into a Sequence from this starter. Do not follow up more than once in seven days without explicit policy. Escalate pricing or commercial decisions instead of inventing discounts.",
    acceptanceCriteria: criteria,
  },
  {
    ...
    brief:
      "Read newly completed meetings and available transcripts from granted calendars. Distinguish agreed commitments from suggestions. Save a concise Workstream with decisions, named owners, due dates, and source meeting IDs. Where you are a Project member, create missing Todos for explicit commitments after checking duplicates; otherwise leave the proposed Todos for a Member. Put the exact recap for verified meeting participants in a Decision-stack mail review. Never fabricate a transcript or email attendees who were not actually present. Mark each processed meeting in the Workstream.",
    acceptanceCriteria: criteria,
  },
  {
    ...
    requirements: ["mail"],
    brief:
      "Review actionable inbound conversations and your open Workstreams. Use the mailbox chosen for this Routine. Prioritize explicit promises, urgent requests, and customer messages unanswered for a business day. Skip spam, newsletters, auto-replies, sent-only threads, and conversations with an existing pending handover or mail review. Put the exact response in the Decision stack or record a specific blocker. Do not send, duplicate another employee's work, or pretend the underlying issue is fixed. Track next checks and close loops once a verified reply or result exists.",
    acceptanceCriteria: criteria,
  },
  {
    ...
    requirements: [],
    brief:
      "Review only your own open Workstreams whose next check is due. Skip records belonging to Improve my work or any suggestion-only review; that Routine alone continues its review. Read the latest source record and do the next authorized step. For Repository work, inspect get_repository_work_session: wait while running, record actionable failure, and publish a ready PR with open_repository_work_session_pull_request only when the Soul explicitly permits it and your Repository write Grant remains active. A stored HTTPS personal access token can push and open the PR without a separate Connection Grant; SSH and Connection-backed repositories require the pinned forge Connection to be granted. Never merge. If a Workstream came from a review-only email handover, preserve review-only customer communication; a later Routine does not upgrade its authority. Review current mail and existing Decision-stack reviews before preparing any update. Record actual links and close completed Workstreams. Leave the next due check in the Workstream for this Routine to revisit. This starter does not create separate Wakeups. Do not poll unchanged work noisily.",
    acceptanceCriteria: criteria,
  },
  {
    id: "improve-own-work",
