...
// The global-SMTP transport is cached and keyed by a signature of the resolved
// settings, so when an operator edits Admin → Email transport the next send
// transparently rebuilds against the new host/credentials without a restart.
let cachedGlobalTransporter: Transporter | null = null;
let cachedGlobalSignature = "";

function globalSmtpSignature(s: ResolvedGlobalSmtp): string {
  return [s.host, s.port, s.secure, s.user, s.pass].join("\u0000");
}

function buildTransport(s: ResolvedGlobalSmtp): Transporter {
  const { secure, requireTLS } = resolveSmtpTransportSecurity(s.port, s.secure);
...
function getGlobalTransporter(s: ResolvedGlobalSmtp): Transporter {
  const sig = globalSmtpSignature(s);
  if (!cachedGlobalTransporter || sig !== cachedGlobalSignature) {
    cachedGlobalTransporter = buildTransport(s);
    cachedGlobalSignature = sig;
  }
...
export async function sendEmail(
  ...
  if (global.configured && global.settings.host) {
    ...
    const sender = global.settings.fromName
      ? { name: global.settings.fromName, address: global.settings.from }
    ...
    try {
      ...
      const message = explainSmtpError(err, global.settings.port);
      const log = await writeLog({
        companyId: opts.companyId ?? null,
        providerId: null,
        transport: "config_smtp",
...
/**
 * Send a test message through the install-wide global SMTP transport, using the
 * draft settings from the Admin → Email transport form (so an operator can
 * validate credentials before saving them, and a blank password field reuses
 * the password already in effect). Logged with `transport: "config_smtp"` and
 * `companyId: null` — this is a system-level, non-company send.
 */
export async function sendGlobalSmtpTest(args: {
  settings: ResolvedGlobalSmtp;
  to: string;
  triggeredByUserId?: string | null;
}): Promise<SendEmailResult> {
  const { settings } = args;
  const subject = "Genosyn — global email transport test";
  const text =
    "This is a test message from Genosyn confirming that your global email " +
    "transport is configured correctly. System emails (password resets, " +
    "invites) will be delivered through this SMTP server.";
  const html =
    "<p>This is a test message from <strong>Genosyn</strong> confirming that your " +
    "global email transport is configured correctly.</p>" +
    "<p>System emails (password resets, invites) will be delivered through this " +
    "SMTP server.</p>";
  try {
    // A one-off transport rather than the cached global one — the draft may not
    // match what is currently saved.
    const from = formatGlobalSmtpSender(settings);
    ...
    const message = explainSmtpError(err, settings.port);
    const log = await writeLog({
      companyId: null,
      providerId: null,
      transport: "config_smtp",
      purpose: "test",
      toAddress: args.to,
      fromAddress: formatGlobalSmtpSender(settings),
      subject,
      bodyPreview: text,
      status: "failed",
      errorMessage: message,
