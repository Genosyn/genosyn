...
export function handoverDeliveryMode(
  mode: MailHandoverMode,
  sourceKind: MailHandoverSource = "manual",
): MailDeliveryMode {
  if (sourceKind === "rule" && mode !== "triage") return "review";
  return mode === "work" || mode === "draft" ? "review" : mode;
}

export function handoverModeGuidance(mode: MailHandoverMode): string {
  switch (mode) {
    case "work":
      return "Mode: WORK. Complete the underlying company work using your granted resources. When the trusted instruction and the customer request warrant a reply, put its exact content in the Decision stack with request_mail_review. Never create a Gmail or IMAP draft and do not send. Spam and newsletter cleanup normally need no reply. For a quote, find the existing Customer first and use verified prices and terms. For a reported product issue, investigate the granted Repository and prepare a reviewable Work session with tests. Never invent prices, claim an unfinished fix is delivered, or merge customer-requested code yourself.";
    case "draft":
      return "Mode: REVIEW. Complete the requested preparation and put the exact reply in the Decision stack with request_mail_review. Do not create a Gmail or IMAP draft and do not send; a Member can edit, send, or discard the reviewed email in Genosyn.";
    case "reply":
      return "Mode: REPLY. Complete the underlying work first. Sending is permitted only when your Soul and the trusted instruction authorize this kind of reply and company Policies and Grants allow it. If those conditions are unclear, use request_mail_review so a human can edit, send, or discard the exact message in Genosyn. Otherwise send with send_mail, preserving the thread and its intended recipients. Never create a Gmail or IMAP draft, and never claim unfinished work is complete.";
    case "triage":
...
export function composeHandoverPrompt(
  ...
  return [
    ...
    "End with a short factual summary and links or ids for the artifacts you actually created. Distinguish Decision-stack email reviews and started Work sessions from sent replies and finished changes.",
  ].join("\n\n");
}
