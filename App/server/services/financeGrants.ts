...
const FINANCE_LEVEL_BLURB: Record<FinanceAccessLevel, string> = {
  ...
  invoice:
    "You can run accounts receivable: create estimate drafts; create, issue, email, and void invoices; list, inspect, create, and update recurring invoice schedules; create and update customers; and record or reverse payments to mark invoices paid. You can also read everything at the `read` level. Estimate drafts have no ledger effect and remain unissued for a Member to review. Attach their returned slug with estimateSlug on create_mail_draft to prepare the quote email; use send_mail only when sending is authorized. Draft PDFs stay visibly DRAFT, and attaching one never issues or accepts the estimate. Issuing an invoice mints its number and posts it to the ledger; sending it emails the customer on file. A recurring schedule can repeat those actions automatically, so choose its status and auto-send setting deliberately.",
